...
const ZERO_DEFAULTS: Record<CategoryKey, CategoryInputs> = {
  isp:    { mode:"flat", units: 0, currentMonthly: 0, proposedMonthly: 0, oneTimeCost: 0, termMonths: 0 },
  voip:   { mode:"flat", units: 0, currentMonthly: 0, proposedMonthly: 0, oneTimeCost: 0, termMonths: 0 },
  wifi:   { mode:"flat", units: 0, currentMonthly: 0, proposedMonthly: 0, oneTimeCost: 0, termMonths: 0 },
  access: { mode:"flat", units: 0, currentMonthly: 0, proposedMonthly: 0, oneTimeCost: 0, termMonths: 0 },
  cloud:  { mode:"flat", units: 0, currentMonthly: 0, proposedMonthly: 0, oneTimeCost: 0, termMonths: 0 },
  mobile: { mode:"flat", units: 0, currentMonthly: 0, proposedMonthly: 0, oneTimeCost: 0, termMonths: 0 },
};
...
export default function CostSavingsCalculator(){
  const [data, setData] = useState<Record<CategoryKey, CategoryInputs>>(()=>{
    const cached = typeof window !== "undefined" ? localStorage.getItem("cocom_calc_v2") : null;
    return cached ? JSON.parse(cached) : ZERO_DEFAULTS;
  });
    ...
    <div className="mx-auto max-w-6xl p-4 md:p-8 text-slate-900 bg-gradient-to-b from-white to-slate-50 rounded-2xl">
      ...
      <Accordion type="single" collapsible className="space-y-4">
        {(Object.keys(data) as CategoryKey[]).map((k)=> (
          <AccordionItem key={k} value={k} className="border-slate-200 rounded-xl overflow-hidden shadow-sm">
            <AccordionTrigger className="px-4 md:px-6 hover:no-underline bg-white data-[state=open]:bg-slate-50 data-[state=open]:text-[#1C3256] transition-colors">
              <div className="flex items-center gap-3 text-left">
                <ChevronRight className="w-4 h-4" style={{color: BRAND.blue}}/>
...
function ExportButtons({data, totals}:{data:Record<CategoryKey, CategoryInputs>, totals: Totals}){
  const toCSV = () => {
    ...
    const csv = rows.map((r) => r.join(",")).join(NL);
  ...
  return (
    <div className="flex items-center gap-2">
      <Button onClick={toCSV} className="bg-[#1C3256] hover:opacity-90">
        <Download className="w-4 h-4 mr-2"/> Export CSV
      </Button>
      <Button variant="secondary" onClick={toJSON} className="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50">Export JSON</Button>
    </div>
  );
}
